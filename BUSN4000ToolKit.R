...
resid_plot <- function(model,
  ...
  # --- helpers ---
  nice_step_min <- function(target) {
    if (!is.finite(target) || target <= 0) return(1)
    m <- 10 ^ floor(log10(target))
    cands <- m * c(1, 2, 5, 10)
    cands[which(cands >= target)[1]]
  }

  make_ticks_nice <- function(minv, maxv, nticks) {
    if (!is.finite(minv) || !is.finite(maxv)) return(NULL)
    if (nticks <= 1) return(mean(c(minv, maxv)))

    rng <- maxv - minv
    if (!is.finite(rng) || rng == 0) {
      # degenerate range: make tiny spread around the value
      return(seq(minv - 1, minv + 1, length.out = nticks))
    }

    step_target <- rng / (nticks - 1)
    step <- nice_step_min(step_target)

    start <- floor(minv / step) * step
    ticks <- start + step * (0:(nticks - 1))

    # If we didn't cover the max, shift right to ensure coverage
    if (max(ticks) < maxv) {
      start2 <- ceiling(maxv / step) * step - step * (nticks - 1)
      ticks <- start2 + step * (0:(nticks - 1))
    }
  ...
  # --- validate inputs ---
  if (!inherits(model, "lm")) stop("model must be an object returned by lm().")
  resid_type <- match.arg(resid_type)
  if (!is.numeric(pad) || length(pad) != 1 || pad < 0) stop("pad must be a single nonnegative number.")
  if (!is.null(ynticks) && (!is.numeric(ynticks) || length(ynticks) != 1 || ynticks < 3)) {
    stop("ynticks must be NULL or a single number >= 3.")
  }
  if (!is.null(xnticks) && (!is.numeric(xnticks) || length(xnticks) != 1 || xnticks < 2)) {
    stop("xnticks must be NULL or a single number >= 2.")
  }
  ...
  if (is.null(ynticks)) {
    # default behavior (pretty + forced symmetry)
    pos_ticks <- pretty(c(0, maxabs))
    pos_ticks <- pos_ticks[pos_ticks >= 0]
    step <- if (length(pos_ticks) >= 2) diff(pos_ticks)[1] else maxabs
    k <- max(1, ceiling(maxabs / step))
    y_ticks <- seq(-k * step, k * step, by = step)
    c_tick <- k * step
  } else {
    ynticks <- as.integer(round(ynticks))
    if (ynticks %% 2 == 0) ynticks <- ynticks + 1  # force odd
    k <- (ynticks - 1) / 2

    step_target <- maxabs / k
    step <- nice_step_min(step_target)
    c_tick <- k * step
    y_ticks <- seq(-c_tick, c_tick, by = step)
  }

  y_lim <- c(-c_tick * (1 + pad), c_tick * (1 + pad))

  # --- x ticks & limits (5% padding) ---
  rx <- range(x, na.rm = TRUE)
  x_rng <- diff(rx)
  if (!is.finite(x_rng) || x_rng == 0) x_rng <- 1
  x_pad <- x_rng * pad
  x_lim <- c(rx[1] - x_pad, rx[2] + x_pad)

  x_ticks <- NULL
  if (!is.null(xnticks)) {
    xnticks <- as.integer(round(xnticks))
    x_ticks <- make_ticks_nice(rx[1], rx[2], xnticks)
  }
  ...
  if (!is.null(x_ticks)) axis(1, at = x_ticks, labels = x_ticks)
...
regprint <- function(model, CL = 95, digits = NULL,
  ...
  if (is_lm) {
    ...
    # Get factor labels if dependent variable is a factor
    # Try model$data first, then model frame
    ...
    if (is.factor(y_orig)) {
      ...
      dv_val <- factor_levels[y_coding + 1]  # +1 because factors are 1-indexed but model$y is 0/1
    } else {
      dv_val <- names(y_table)
    }
